...
public class GenericClusterManagerTest {

    private ClusterManager<FeatureVector> clusterManager;

    private List<FeatureVector> featureVectors;

    private final int clusterTargetCount = 4;
    private final int vectorSize = 10;

    @Before
    public void init() {
        clusterManager = new GenericClusterManager<>();
        ...
        featureVectors = Arrays.asList(tags1, tags2, tags3, tags4, tags5, tags6).stream()
    ...
    public void testGetClusters() {
        ...
        final Collection<? extends Cluster<FeatureVector>> clusters = clusterer.cluster(featureVectors);
        clusterManager.addClusters(clusters);

        final Collection<? extends Cluster<FeatureVector>> managedClusters = clusterManager.getClusters();

        assertThat(clusters, is(managedClusters));
    }

    @Test
    public void testGetNearestCluster() {
        final FeatureVector featureVector = createStringFeatureVector(Arrays.asList("B", "C", "D"), vectorSize);
        ...
        final Cluster nearestCluster = clusterManager.getNearestCluster(featureVector);

        assertThat(nearestCluster, notNullValue());
    }
